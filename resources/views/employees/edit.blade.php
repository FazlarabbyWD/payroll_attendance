                ...
                <ul class="nav nav-pills flex-row">
                    <li class="nav-item">
                        <a class="nav-link active waves-effect waves-light" href="javascript:void(0);">
                            <i class="icon-base ri ri-group-line icon-sm me-1_5"></i>Edit Employee
                        </a>
                    </li>
                </ul>
                            ...
                            <div class="col-md-6">
                                <div class="form-floating form-floating-outline">
                                    <select id="employment_type_id" name="employment_type_id"
                                        ...
                                        <option value="{{ $employeeType->id }}" {{ old('employment_type_id', $employee->
                                            employment_type_id) == $employeeType->id ? 'selected' : '' }}>
                                            {{ $employeeType->name }}
                                        </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-floating form-floating-outline">
                                    <select id="department_id" name="department_id"
                                        ...
                                        <option value="{{ $department->id }}" {{ old('department_id', $employee->
                                ...
                                <div class="form-control @error('company_ids') is-invalid @enderror"
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="company_ids[]"
                                            ...
                                            (collect(old('company_ids',
                                            $employee->companies->pluck('id')->toArray()))->contains($company->id)) ?
                                        'checked' : '' }}>
...
<script>
    ...
    var selectedDesignationId = "{{ old('designation_id', $employee->designation_id) }}";
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        data.forEach(function (designation) {
                            let selected = selectedDesignationId == designation.id ? 'selected' : '';
                            options += `<option value="${designation.id}" ${selected}>${designation.name}</option>`;
                        });
